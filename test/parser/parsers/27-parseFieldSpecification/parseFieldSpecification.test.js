...
describe("field-specification, valid inputs", () => {
    runTestSuitesToEqual(
        "./test/parser/parsers/27-parseFieldSpecification/testCases",
        /^test-suite-/,
        (input) => {
            let tokens = lex(input);
            tokens = filterTokens(tokens);
            const output = parsers.parseFieldSpecification(tokens, 0);
            return output;
        },
    );
});
