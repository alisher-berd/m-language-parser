...
describe("required-projection, valid inputs", () => {
    runTestSuitesToEqual(
        "./test/parser/parsers/19-parseRequiredProjection/testCases",
        /^test-suite-/,
        (input) => {
            let tokens = lex(input);
            tokens = filterTokens(tokens);
            const output = parsers.parseRequiredProjection(tokens, 0);
            return output;
        },
    );
});
