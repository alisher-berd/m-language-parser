...
describe("parameter-specification, valid inputs", () => {
    runTestSuitesToEqual(
        "./test/parser/parsers/35-parseParameterSpecification/testCases",
        /^test-suite-/,
        (input) => {
            let tokens = lex(input);
            tokens = filterTokens(tokens);
            const output = parsers.parseParameterSpecification(tokens, 0);
            return output;
        },
    );
});
