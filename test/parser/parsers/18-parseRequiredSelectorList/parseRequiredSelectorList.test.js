...
describe("required-selector-list, valid inputs", () => {
    runTestSuitesToEqual(
        "./test/parser/parsers/18-parseRequiredSelectorList/testCases",
        /^test-suite-/,
        (input) => {
            let tokens = lex(input);
            tokens = filterTokens(tokens);
            const output = parsers.parseRequiredSelectorList(tokens, 0);
            return output;
        },
    );
});
