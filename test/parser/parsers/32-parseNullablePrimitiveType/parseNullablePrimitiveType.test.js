...
describe("nullable-primitive-type, valid inputs", () => {
    runTestSuitesToEqual(
        "./test/parser/parsers/32-parseNullablePrimitiveType/testCases",
        /^test-suite-/,
        (input) => {
            let tokens = lex(input);
            tokens = filterTokens(tokens);
            const output = parsers.parseNullablePrimitiveType(tokens, 0);
            return output;
        },
    );
});
